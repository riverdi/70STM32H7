...
void BSP_LCD_Reset(uint32_t Instance)
{
  ...
  HAL_GPIO_WritePin(LCD_DISP_GPIO_Port , LCD_DISP_Pin, GPIO_PIN_SET);/* Deactivate XRES */
...
void MX_DSIHOST_DSI_Init(void)
{
  ...
  uint32_t LcdClock  = 41666; /*!< LcdClk = 44833 kHz */

  uint32_t laneByteClk_kHz = 62000; /* 500 MHz / 8 = 62.5 MHz = 62500 kHz */
  ...
  VidCfg.NullPacketSize = 0x0;
  VidCfg.HSPolarity = DSI_HSYNC_ACTIVE_LOW;
  VidCfg.VSPolarity = DSI_VSYNC_ACTIVE_LOW;
  VidCfg.DEPolarity = DSI_DATA_ENABLE_ACTIVE_HIGH;
  VidCfg.HorizontalSyncActive =   HSYNC * laneByteClk_kHz/LcdClock; //HSYNC;
  VidCfg.HorizontalBackPorch = (uint32_t) (HBP * laneByteClk_kHz/LcdClock); //HBP ;
  ...
  HAL_DSI_Start(&hdsi);
   ...
   //     (void)HAL_DSI_ConfigFlowControl(&hlcd_dsi, DSI_FLOW_CONTROL_BTA);
        ...
        HAL_DSI_ShortWrite(&hdsi,0,DSI_DCS_SHORT_PKT_WRITE_P0,DSI_SOFT_RESET,0x0); //software reset
             HAL_Delay(120);
             //HAL_DSI_LongWrite(&hlcd_dsi, 0, DSI_DCS_LONG_PKT_WRITE, Size, (uint32_t)Reg, pData);
             HAL_DSI_ShortWrite(&hdsi, 0, DSI_GEN_SHORT_PKT_WRITE_P2, 0x87, 0x5A); // unlocks control registers
             ...
             HAL_DSI_ShortWrite(&hdsi,0,DSI_DCS_SHORT_PKT_WRITE_P0,DSI_EXIT_SLEEP_MODE,0x0); //sleep off
             HAL_Delay(120);
